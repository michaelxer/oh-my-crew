...
describe("migrateAgentNames", () => {
  ...
  test("migrates Prometheus variants to lowercase", () => {
    // given agents config with "Architect - Plan Builder" key
    // when migrateAgentNames called
    // then key becomes "prometheus"
    const agents = { "Architect - Plan Builder": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["prometheus"]).toEqual({ model: "test" })
    expect(migrated["Architect - Plan Builder"]).toBeUndefined()
  })

  test("migrates Metis variants to lowercase", () => {
    // given agents config with "Advisor - Plan Consultant" key
    // when migrateAgentNames called
    // then key becomes "metis"
    const agents = { "Advisor - Plan Consultant": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["metis"]).toEqual({ model: "test" })
    expect(migrated["Advisor - Plan Consultant"]).toBeUndefined()
  })

  test("migrates Momus variants to lowercase", () => {
    // given agents config with "Auditor - Plan Critic" key
    // when migrateAgentNames called
    // then key becomes "momus"
    const agents = { "Auditor - Plan Critic": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["momus"]).toEqual({ model: "test" })
    expect(migrated["Auditor - Plan Critic"]).toBeUndefined()
  })
