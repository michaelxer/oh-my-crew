...
function withDefaultStdio(options: SpawnOptions): SpawnOptions {
  if (options.stdio || options.stdin) return options
...
export function spawn(cmdOrOpts: unknown, opts?: unknown): SpawnedProcess {
  ...
  if (IS_BUN) {
    return runtime.Bun!.spawn(cmd, withDefaultStdio(options))
  }
