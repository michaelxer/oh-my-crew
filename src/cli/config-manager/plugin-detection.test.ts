...
describe("detectCurrentConfig - single package detection", () => {
  ...
  it("detects both legacy and canonical plugin entries", () => {
    // given
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["oh-my-opencode", "oh-my-openagent@3.11.0", "oh-my-crew"] }, null, 2) + "\n", "utf-8")
...
describe("addPluginToOpenCodeConfig - single package writes", () => {
  ...
  it("updates a version-pinned legacy entry to the requested version", async () => {
    // given
    const getPluginNameWithVersionSpy = spyOn(pluginNameWithVersion, "getPluginNameWithVersion").mockResolvedValue("oh-my-crew@3.16.0")
    ...
    expect(result.success).toBe(true)
    const savedConfig = JSON.parse(readFileSync(testConfigPath, "utf-8"))
    expect(savedConfig.plugin).toEqual(["oh-my-crew@3.16.0"])
    getPluginNameWithVersionSpy.mockRestore()
  })

  it("removes stale legacy entry when canonical and legacy entries both exist", async () => {
    // given
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["oh-my-crew", "oh-my-openagent", "oh-my-opencode", "oh-my-china"] }, null, 2) + "\n", "utf-8")
  ...
  it("preserves a canonical entry when the same version is re-installed", async () => {
    // given
    const getPluginNameWithVersionSpy = spyOn(pluginNameWithVersion, "getPluginNameWithVersion").mockResolvedValue("oh-my-crew@3.10.0")
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["oh-my-crew@3.10.0"] }, null, 2) + "\n", "utf-8")
    ...
    expect(result.success).toBe(true)
    const savedConfig = JSON.parse(readFileSync(testConfigPath, "utf-8"))
    expect(savedConfig.plugin).toEqual(["oh-my-crew@3.10.0"])
    getPluginNameWithVersionSpy.mockRestore()
  })

  it("blocks a downgrade for a version-pinned canonical entry", async () => {
    // given
    const getPluginNameWithVersionSpy = spyOn(pluginNameWithVersion, "getPluginNameWithVersion").mockResolvedValue("oh-my-crew@3.15.0")
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["oh-my-crew@3.16.0"] }, null, 2) + "\n", "utf-8")
