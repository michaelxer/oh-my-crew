...
export function createMultimodalLookerAgent(model: string): AgentConfig {
  ...
  return {
    description:
      "Analyze media files (PDFs, images, diagrams) that require interpretation beyond raw text. Extracts specific information or summaries from documents, describes visual content. Use when you need analyzed/extracted data rather than literal file contents. (Lookout - OhMyCrew)",
    mode: MODE,
    model,
    temperature: 0.1,
    ...restrictions,
