...
import { open, readFile, rename, rm, unlink, writeFile, type FileHandle } from "node:fs/promises"
...
async function syncFileHandle(fileHandle: FileHandle): Promise<void> {
  try {
  ...
  for (;;) {
    ...
    try {
      ...
      try {
        ...
        await syncFileHandle(fileHandle)
      } finally {
